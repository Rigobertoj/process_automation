...
class Process_data():
    ...
    def process_xml_data(self,*data ):
        *_, data = data
        ...
        if "Traslado 16" in data["Mount"]:
            data_dict["IVA"] = data["Mount"]["Traslado 16"]
        if "Retenciones IVA" in data["Mount"]:
            data_dict["Ret. IVA."] = data["Mount"]["Retenciones IVA"]
        if "Ret. ISR" in data["Mount"]:
            data_dict["Ret. ISR."] = data["Mount"]["Retenciones ISR"]
    ...
    def get_data(self) -> list[list, list]:
        ...
        for file in file_xml:
            ...
            data_process_xml = self.process_xml_data(xml_data)
            data.append(data_process_xml)
